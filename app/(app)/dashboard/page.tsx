...
export default function DashboardPage() {
  ...
  return <div className="space-y-6">
    ...
    <section className="grid gap-5 xl:grid-cols-3">
      ...
      <div className="card flex min-h-[350px] flex-col items-center text-center">
        <div className="flex w-full items-center justify-between">
          <div className="text-left">
            <h3 className="section-title">Focus Timer</h3>
            <p className="section-copy">Pomodoro session</p>
          </div>
          <span className="rounded-lg bg-brand-50 p-2 text-brand-500 dark:bg-brand-500/10">
            <Timer size={17} />
          </span>
        </div>

        <div className="relative mt-6 grid h-44 w-44 place-items-center rounded-full timer-ring" style={{ '--ring-progress': `${((1500 - seconds) / 1500) * 100}%` } as CSSProperties}>
          <div className="grid h-[calc(100%-14px)] w-[calc(100%-14px)] place-items-center rounded-full bg-white dark:bg-[#11182b]">
            <div>
              <b className="block text-3xl tracking-tight">{timer}</b>
              <span className="text-[10px] font-bold uppercase tracking-[.16em] text-slate-400">{running ? 'Focus in progress' : 'Ready to focus'}</span>
            </div>
          </div>
        </div>

        <button onClick={() => setRunning(!running)} className="mt-6 inline-flex items-center gap-2 rounded-xl bg-brand-500 px-5 py-2.5 text-sm font-semibold text-white shadow-lg shadow-brand-500/25">
          {running ? <Clock3 size={16} /> : <Play size={16} className="fill-current" />}
          {running ? 'Pause Focus' : 'Start Focus'}
        </button>
        <p className="mt-3 text-[11px] text-slate-400">Next break in 25 minutes</p>
      </div>
    </section>
    ...
    <section className="grid gap-5 xl:grid-cols-3"><div className="relative overflow-hidden rounded-2xl bg-gradient-to-br from-brand-600 via-violet-600 to-indigo-700 p-5 text-white shadow-xl shadow-brand-500/20"><Sparkles className="absolute -right-3 -top-3 h-28 w-28 text-white/10"/><span className="inline-flex items-center gap-1.5 rounded-full bg-white/15 px-2.5 py-1 text-[10px] font-bold uppercase tracking-wider"><Sparkles size={13}/> AI suggestion</span><h3 className="mt-4 text-lg font-bold">Your best focus window is now.</h3><p className="mt-2 text-xs leading-relaxed text-violet-100">You&apos;re usually 24% more productive between 10 AM and noon. Start with the mobile wireframes while your energy is high.</p><button className="mt-5 inline-flex items-center gap-1.5 text-xs font-bold text-white">Use this plan <ArrowUpRight size={14}/></button></div>
      ...
      <div className="card"><div className="mb-3 flex items-center justify-between"><div><h3 className="section-title">Productivity Analytics</h3><p className="section-copy">Focus score this week</p></div><span className="inline-flex items-center gap-1 text-xs font-bold text-emerald-500"><TrendingUp size={14}/> 12%</span></div><div className="h-28"><ResponsiveContainer width="100%" height="100%"><AreaChart data={chartData}><defs><linearGradient id="productivity" x1="0" x2="0" y1="0" y2="1"><stop stopColor="#6c4cfd" stopOpacity=".35"/><stop offset="1" stopColor="#6c4cfd" stopOpacity="0"/></linearGradient></defs><Tooltip /><XAxis dataKey="day" axisLine={false} tickLine={false} tick={{fontSize:10,fill:'#94a3b8'}}/><Area type="monotone" dataKey="value" stroke="#6c4cfd" strokeWidth={3} fill="url(#productivity)"/></AreaChart></ResponsiveContainer></div><div className="mt-3 grid grid-cols-3 border-t border-slate-100 pt-3 text-center dark:border-white/10"><span><b className="block text-sm">82%</b><small className="text-[10px] text-slate-400">Focus score</small></span><span><b className="block text-sm">18</b><small className="text-[10px] text-slate-400">Tasks done</small></span><span><b className="block text-sm">8.4h</b><small className="text-[10px] text-slate-400">Deep work</small></span></div></div></section>
  </div>;
}
